...
const { verifyMessageMock } = require("./sign");
...
class EXM {
  static async deterministicFetch(url, options = {}) {
...
async function handle(state, action) {
  ...
  async function _authenticate(caller, message, signature) {
    try {
      const recovered = verifyMessageMock(caller, message, signature);
      ContractAssert(
        recovered.toLowerCase() === caller.toLowerCase(),
